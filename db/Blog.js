...
var BlogSql={
  ...
  /*  readPartComm:"SELECT o.* FROM (SELECT * FROM comment WHERE parent_id=? AND parent_type=? order by id desc) o limit ?,10",*/
    ...
    searchBlog:"SELECT  img,name,blog.sendername,blog.likes,blog.time,blog.sendername,blog.id,blog.content,blog.title FROM user RIGHT JOIN blog ON user.username=blog.sendername WHERE blog.title LIKE ? OR blog.content LIKE ? OR blog.sendername LIKE ?  ORDER BY id DESC limit 0,20",
